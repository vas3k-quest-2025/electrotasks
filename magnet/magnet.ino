...
static const int HALL_PIN = 3;
static const unsigned long MAGNET_DISPLAY_DURATION = 5000; // 5 секунд в миллисекундах
static const String CODE_WORD = "СКЛОННОСТЬ";
...
void setup() {
  ...
  lastMagnetTime = millis() - MAGNET_DISPLAY_DURATION;
...
void loop() {
  ...
  static String code;
  const int hallValue = digitalRead(HALL_PIN);
  unsigned long currentTime = millis();

  if (hallValue == LOW || currentTime - lastMagnetTime < MAGNET_DISPLAY_DURATION) {
    feeling = "Я чувствую\nвлечение!";
    code = "Код:\n" + CODE_WORD;
    if (hallValue == LOW) {
