...
const static String code = "ЗЕЛЬЕ";
const static int SENSOR_THRESHOLD_HIGH = 2000; // Порог для перехода в SHOWING_CODE
const static int SENSOR_THRESHOLD_LOW = 1950;  // Порог для перехода в WAITING
...
{       
    ...
    lv_label_set_text(uic_message, "Мне нужно\nпередохнуть,\nподожди");
    Set_Backlight(30);
    ...
    switch (currentState) {
        case WARMING:
            if (sensorValue < SENSOR_THRESHOLD_LOW) {
                ...
                lv_label_set_text(uic_message, "ну-ка дыхни");
            }
            break;
